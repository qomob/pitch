...
英文模式额外加载: [bilingual-templates.md](references/bilingual-templates.md)

## 执行规则

**核心机制：** 执行每个Agent前，先读取对应的 `agents/<name>.md` 和 `references/` 文件。Agent文件包含完整的执行流程、输出格式和降级策略，此处不重复。

**依赖链：** Intake → Information → Strategy → Decision → Expression → Delivery
